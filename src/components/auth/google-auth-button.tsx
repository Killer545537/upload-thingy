import { useMutation, useQueryClient } from '@tanstack/react-query';
import { motion } from 'framer-motion';
import { gooeyToast } from 'goey-toast';
import { Button } from '#/components/ui/button';
import { authClient } from '#/lib/auth/client';
import { authQueries } from '#/queries/auth';

const GoogleAuthButton = () => {
    const queryClient = useQueryClient();

    const googleMutation = useMutation({
        mutationFn: async () => {
            const { error } = await authClient.signIn.social({
                callbackURL: '/dashboard',
                provider: 'google',
            });

            if (error) {
                throw new Error(
                    error.message ||
                        'An unknown error occurred during Google sign in.',
                );
            }
        },
        onSuccess: () => {
            // Invalidate auth queries to refetch user session
            queryClient.invalidateQueries({ queryKey: authQueries.all });
            // Note: The redirect to /dashboard is handled by callbackURL
        },
        onError: (error) => {
            gooeyToast.error(error.message);
        },
    });

    return (
        <motion.div
            animate='visible'
            initial='hidden'
            transition={{ duration: 0.5, ease: 'easeOut' }}
            variants={{
                hidden: { opacity: 0, y: 20 },
                visible: { opacity: 1, y: 0 },
            }}
        >
            <Button
                className='w-full h-12 gap-3 text-foreground border-border hover:bg-secondary transition-all duration-300'
                onClick={() => googleMutation.mutate()}
                disabled={googleMutation.isPending}
                variant='outline'
            >
                <svg className='w-5 h-5' viewBox='0 0 24 24'>
                    <path
                        d='M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z'
                        fill='currentColor'
                    />
                    <path
                        d='M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z'
                        fill='currentColor'
                    />
                    <path
                        d='M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z'
                        fill='currentColor'
                    />
                    <path
                        d='M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z'
                        fill='currentColor'
                    />
                </svg>
                {googleMutation.isPending
                    ? 'Connecting...'
                    : 'Continue with Google'}
            </Button>
        </motion.div>
    );
};

export default GoogleAuthButton;
